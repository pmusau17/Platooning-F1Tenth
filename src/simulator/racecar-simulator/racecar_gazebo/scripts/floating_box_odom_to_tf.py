...
class BoxOdomToTF:
    def __init__(self,box_name):
        ...
        self.brodcaster = tf2_ros.TransformBroadcaster()
        self.box_name = box_name

        self.sub = rospy.Subscriber("/"+self.box_name+"/odom",Odometry,self.vesc_odom_callback)
    def vesc_odom_callback(self,data):
        ...
        odom_trans.header.frame_id=self.box_name+"/odom"
        odom_trans.child_frame_id=self.box_name+"/base_footprint"
        ...
        self.brodcaster.sendTransform(odom_trans)
        ...
        tf1 = TransformStamped(
                ...
                child_frame_id=self.box_name+"/odom",
                transform=Transform(
                    translation=pose2,
                    rotation=Quaternion(*tf.transformations.quaternion_from_euler(0,0,0))
                )
            )
        self.brodcaster.sendTransform(tf1)
