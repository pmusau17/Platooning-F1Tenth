...
double startState[2][2] = {{0.0, 0.0}, {0.0, 0.0}};
double vxi[2] = {0.0,0.0};
double vyi[2] = {0.0,0.0};
...
void timer_callback(const ros::TimerEvent& event)
{
  ...
  double x,y,vx,vy;
  double pose_up, pose_down, vel_up, vel_down;  
  ...
  if(count>0)
  {
    ...
    // uncertainty associated with position and velocity
    // respectively
    pose_up = (1+position_uncertainty);
    pose_down = (1-position_uncertainty);
    vel_up = (1+velocity_uncertainty);
    vel_down = (1-velocity_uncertainty);

    // compute initial state
    startState[0][0] = pose_down * x;
    startState[0][1] = pose_up * x;
    startState[1][0] = pose_down * y;
    startState[1][1] = pose_up * y;
    ...
    HyperRectangle reach_hull = runReachability_obstacle_uncertain(startState,sim_time, walltime, 0,vxi,vyi,hr_list,&rect_count,max_hyper_rectangles,true);
  ...
  // publish an empty message if there is no odometry message
  else
  {
      rtreach::reach_tube reach_set;
      reach_set.header.stamp = ros::Time::now();
...
int main(int argc, char **argv)
{
    ...
    if(argv[5] == NULL)
    {
        std::cout << "Please enter the position uncertainty that you are considering for the opponents" << std::endl;
        exit(0);
    }

    if(argv[6] == NULL)
    {
        std::cout << "Please enter the velocity uncertainty that you are considering" << std::endl;
        exit(0);
    }
    ...
    position_uncertainty = atof(argv[5]);
    velocity_uncertainty = atof(argv[6]);


    // initialize the ros node
    ros::init(argc, argv, "visualize_node_uncertain");
