...
void make_neighborhood_rect_bicycle_dyn(HyperRectangle* out, int f, HyperRectangle* bloatedRect, HyperRectangle* originalRect, REAL nebWidth)
{
	*out = *bloatedRect;
...
REAL lift_single_rect_bicycle_dyn(HyperRectangle* rect, REAL stepSize, REAL timeRemaining, REAL heading_input, REAL throttle)
{
	...
	while (needRecompute)
	{
		...
		for (int f = 0; f < NUM_FACES; ++f)
		{
			...
			make_neighborhood_rect_bicycle_dyn(&faceNebRect, f, &bloatedRect, rect, nebWidth[f]);
...
bool face_lifting_iterative_improvement_bicycle_dyn(int startMs, LiftingSettings* settings,REAL heading_input, REAL throttle,HyperRectangle VisStates[],int  *total_intermediate,int max_intermediate,bool plot)
{
	...
	while (true)
	{
		...
		while (safe && timeRemaining > 0)
		{
			...
			REAL timeElapsed = lift_single_rect_bicycle_dyn(&trackedRect, stepSize, timeRemaining, heading_input, throttle);
