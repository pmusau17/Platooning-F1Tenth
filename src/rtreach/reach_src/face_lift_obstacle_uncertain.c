...
REAL get_derivative_bounds_obstacle_uncertain(HyperRectangle* rect, int faceIndex, REAL v_x[2], REAL v_y[2]);
...
REAL lift_single_rect_obstacle_uncertain(HyperRectangle* rect, REAL stepSize, REAL timeRemaining, REAL v_x[2], REAL v_y[2])
{
	...
	while (needRecompute)
	{
		...
		for (int f = 0; f < NUM_FACES; ++f)
		{
			...
			REAL der = get_derivative_obstacle_uncertain(&faceNebRect,f, v_x, v_y);
...
HyperRectangle face_lifting_iterative_improvement_uncertain(int startMs, LiftingSettings* settings, REAL v_x[2], REAL v_y[2], HyperRectangle VisStates[],int  *total_intermediate,int max_intermediate,bool plot)
{
	...
	while (true)
	{
		...
		while (timeRemaining > 0)
		{
			...
			REAL timeElapsed = lift_single_rect_obstacle_uncertain(&trackedRect, stepSize, timeRemaining, v_x, v_y);
