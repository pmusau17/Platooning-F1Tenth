...
HyperRectangle face_lifting_iterative_improvement_obstacle_vis(int startMs, LiftingSettings* settings, REAL v_x, REAL v_y, HyperRectangle VisStates[],int  *total_intermediate,int max_intermediate,bool plot);
...
void make_neighborhood_rect_obstacle(HyperRectangle* out, int f, HyperRectangle* bloatedRect, HyperRectangle* originalRect, REAL nebWidth)
{
	*out = *bloatedRect;
...
REAL lift_single_rect_obstacle(HyperRectangle* rect, REAL stepSize, REAL timeRemaining, REAL v_x, REAL v_y)
{
	...
	while (needRecompute)
	{
		...
		for (int f = 0; f < NUM_FACES; ++f)
		{
			...
			make_neighborhood_rect_obstacle(&faceNebRect, f, &bloatedRect, rect, nebWidth[f]);
			
			// test derivative inside neighborhood

			// the projection of the derivative on the outward norm e_i- is -fi(x) and fi(x) for e_i
			REAL der = get_derivative_obstacle(&faceNebRect,f, v_x, v_y);
...
HyperRectangle face_lifting_iterative_improvement_obstacle_vis(int startMs, LiftingSettings* settings, REAL v_x, REAL v_y, HyperRectangle VisStates[],int  *total_intermediate,int max_intermediate,bool plot)
{
	...
	while (true)
	{
		...
		while (timeRemaining > 0)
		{
			...
			REAL timeElapsed = lift_single_rect_obstacle(&trackedRect, stepSize, timeRemaining, v_x, v_y);
