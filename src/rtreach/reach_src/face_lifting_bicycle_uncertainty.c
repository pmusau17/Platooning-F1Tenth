...
const REAL MAX_DER_U = 99999;
const REAL MIN_DER_U = -99999;
...
void make_neighborhood_rect_bicycle_uncertain(HyperRectangle* out, int f, HyperRectangle* bloatedRect, HyperRectangle* originalRect, REAL nebWidth)
{
	*out = *bloatedRect;
...
REAL lift_single_rect_bicycle_uncertain(HyperRectangle* rect, REAL stepSize, REAL timeRemaining, REAL heading_input, REAL throttle, REAL parameter_uncertainty, REAL disturbance[][2])
{
	////////////////////////////////////////////////
	...
	while (needRecompute)
	{
		...
		for (int f = 0; f < NUM_FACES; ++f)
		{
			...
			make_neighborhood_rect_bicycle_uncertain(&faceNebRect, f, &bloatedRect, rect, nebWidth[f]);
			
			// test derivative inside neighborhood

			// the projection of the derivative on the outward norm e_i- is -fi(x) and fi(x) for e_i
			REAL der = get_derivative_bounds_bicycle_uncertainty(&faceNebRect,f, heading_input, throttle,parameter_uncertainty,disturbance);
...
bool face_lifting_iterative_improvement_bicycle_uncertainty(int startMs, LiftingSettings* settings, REAL heading_input, REAL throttle,
                                                            REAL parameter_uncertainty,REAL disturbance[][2], HyperRectangle VisStates[],
                                                            int  *total_intermediate,int max_intermediate,bool plot)
{
	bool rv = false;
	...
	while (true)
	{
		...
		{
			settings->restartedComputation(total_intermediate);
        }
		...
		while (safe && timeRemaining > 0)
		{
			...
			REAL timeElapsed = lift_single_rect_bicycle_uncertain(&trackedRect, stepSize, timeRemaining, heading_input, throttle, parameter_uncertainty,disturbance);
			...
			else if (settings->reachedAtIntermediateTime)
			{
				...
				safe = safe && settings->reachedAtIntermediateTime(&hull,VisStates,total_intermediate,max_intermediate);
			}

			if (timeElapsed == timeRemaining && settings->reachedAtFinalTime)
				safe = safe && settings->reachedAtFinalTime(&trackedRect,VisStates,total_intermediate,max_intermediate);
		...
		if (settings->maxRuntimeMilliseconds > 0)
		{
			...
			if (remaining < 0)
			{
                ...
                if(plot)
                {
                    settings->reachedAtFinalTime(&total_hull,VisStates,total_intermediate,max_intermediate); // purely for plotting purposes
                }
				//println(&trackedRect);
				break;
			}
