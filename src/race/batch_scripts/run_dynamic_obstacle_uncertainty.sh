...
do 
    ...
    do
        ...
        do
            ...
            do
            ((count=count+1)) 
            roslaunch race sim_for_rtreach_dynamic_obstacle_uncertainty.launch timeout:=$timeout number_of_cars:=$cars num_obstacles:=0 position_uncertainty:=$posu velocity_uncertainty:=$velu experiment_number:=$count & 
