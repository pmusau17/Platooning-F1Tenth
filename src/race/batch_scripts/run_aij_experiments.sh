        ...
        do
            ...
            for vel in $velocities
                ...
                for cntrl in $control
                        ...
                        do
                        ((count=count+1)) 
                        roslaunch race sim_for_rtreach_aij_experiments.launch algorithm:=$alg control:=$cntrl timeout:=$timeout velocity:=$vel number_of_cars:=$cars num_obstacles:=$obs experiment_number:=$count & 
